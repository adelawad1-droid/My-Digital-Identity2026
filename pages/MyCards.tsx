...
const MyCards: React.FC<MyCardsProps> = ({ lang, cards, onAdd, onEdit, onDelete }) => {
  ...
  const [maxCardsLimit, setMaxCardsLimit] = useState(5); 
    ...
    <div className="max-w-7xl mx-auto px-4 space-y-12 animate-fade-in-up pb-32">
      ...
      <div className="w-full max-w-2xl mx-auto">
         <div className="relative group">
            ...
            />
            {searchQuery && (
              <button onClick={() => setSearchQuery('')} className={`absolute inset-y-0 ${isRtl ? 'left-4' : 'right-4'} flex items-center text-gray-400 hover:text-red-500 transition-colors`}>
                 <X size={18} />
              </button>
            )}
         </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
                ...
                <div className="flex items-center gap-4">
                  <div className="relative">
                    <div className="w-16 h-16 rounded-[1.5rem] bg-gray-50 dark:bg-gray-800 border-2 border-white dark:border-gray-700 shadow-lg overflow-hidden flex items-center justify-center">
                      {card.profileImage ? <img src={card.profileImage} className="w-full h-full object-cover" alt="Profile" /> : <UserIcon size={24} className="text-gray-300"/>}
                    </div>
                    {card.isActive !== false && <div className="absolute -top-1 -right-1 w-4 h-4 bg-emerald-500 border-2 border-white dark:border-[#0f0f12] rounded-full"></div>}
                  </div>
              ...
              <div className="px-8 py-4 relative">
                {card.showMembership && mStats ? (
                  <div className="p-5 bg-gray-50 dark:bg-white/[0.03] rounded-[2rem] border border-gray-100 dark:border-white/5 space-y-4 group/membership relative">
                    <div className="flex justify-between items-center">
                       <div className="flex items-center gap-2">
                          ...
                          <span className="text-[9px] font-black dark:text-white uppercase tracking-widest">{isRtl ? (card.membershipTitleAr || 'حالة العضوية') : (card.membershipTitleEn || 'Membership')}</span>
                       </div>
                       <span className={`text-[9px] font-black px-2 py-0.5 rounded-full ${mStats.daysLeft > 0 ? 'bg-emerald-50 text-emerald-600' : 'bg-red-50 text-red-600'}`}>{mStats.daysLeft > 0 ? (isRtl ? `${mStats.daysLeft} يوم متبقي` : `${mStats.daysLeft} Days Left`) : (isRtl ? 'منتهي' : 'Expired')}</span>
                    </div>
                    <div className="h-2 w-full bg-gray-200 dark:bg-white/10 rounded-full overflow-hidden"><div className={`h-full transition-all duration-1000 ${mStats.color}`} style={{ width: `${mStats.progress}%` }} /></div>
                    ...
                    {isPremium && (
                      <button onClick={() => openMembershipManager(card)} className="absolute inset-0 bg-blue-600/90 text-white rounded-[2rem] flex flex-col items-center justify-center gap-2 opacity-0 group-hover/membership:opacity-100 transition-opacity duration-300 backdrop-blur-sm">
                         <ShieldCheck size={28} /><span className="text-[10px] font-black uppercase tracking-widest">{isRtl ? 'إدارة سريعة للعضوية' : 'Manage Membership'}</span>
                      </button>
                    )}
                  </div>
                ) : (
                  <div className="h-[102px] flex items-center justify-center border border-dashed border-gray-200 dark:border-gray-800 rounded-[2rem] group-hover:border-blue-500/30 transition-colors relative group/membership">
                     ...
                     {isPremium && (
                      <button onClick={() => openMembershipManager(card)} className="absolute inset-0 bg-blue-600/90 text-white rounded-[2rem] flex flex-col items-center justify-center gap-2 opacity-0 group-hover/membership:opacity-100 transition-opacity duration-300 backdrop-blur-sm">
                         <ShieldCheck size={28} /><span className="text-[10px] font-black uppercase tracking-widest">{isRtl ? 'تفعيل العضوية الآن' : 'Enable Membership Now'}</span>
                      </button>
                    )}
                  </div>
                )}
        ...
        {filteredCards.length === 0 && cards.length > 0 && (
           <div className="col-span-full py-20 flex flex-col items-center justify-center text-center space-y-6 bg-white dark:bg-[#0f0f12] rounded-[3rem] border-2 border-dashed border-gray-100 dark:border-gray-800">
              <div className="w-20 h-20 bg-gray-50 dark:bg-gray-800 rounded-full flex items-center justify-center text-gray-300"><FolderSearch size={40} /></div>
              <div><h3 className="text-xl font-black dark:text-white">{isRtl ? 'لا توجد نتائج مطابقة' : 'No matching results'}</h3><p className="text-gray-400 font-bold text-sm mt-2">{isRtl ? `لم نجد أي بطاقة تحتوي على اسم "${searchQuery}"` : `No cards found matching "${searchQuery}"`}</p></div>
              <button onClick={() => setSearchQuery('')} className="px-8 py-3 bg-blue-50 dark:bg-blue-900/20 text-blue-600 rounded-xl font-black text-xs uppercase hover:bg-blue-600 hover:text-white transition-all">{isRtl ? 'عرض كافة البطاقات' : 'Show All Cards'}</button>
           </div>
        )}
        ...
        {cards.length === 0 && (
          <button onClick={handleAddNewCard} className="flex flex-col items-center justify-center p-12 rounded-[3rem] border-2 border-dashed border-gray-200 dark:border-gray-800 hover:border-blue-500 hover:bg-blue-50/10 transition-all duration-500 group">
            <div className="w-20 h-20 rounded-[2rem] bg-gray-50 dark:bg-gray-800 flex items-center justify-center text-gray-400 group-hover:bg-blue-600 group-hover:text-white transition-all shadow-inner mb-6"><Plus size={32} /></div>
            <span className="text-xs font-black text-gray-400 group-hover:text-blue-600 uppercase tracking-widest">{isRtl ? 'إضافة بطاقة جديدة' : 'Add New Card'}</span>
          </button>
        )}
      </div>
        ...
        <div className="fixed inset-0 z-[2000] flex items-center justify-center bg-black/80 backdrop-blur-md p-4 animate-fade-in">
           <div className="bg-white dark:bg-gray-900 w-full max-w-lg rounded-[3.5rem] shadow-0 border border-gray-100 dark:border-gray-800 overflow-hidden flex flex-col animate-zoom-in">
              <div className="p-8 bg-blue-600 text-white flex justify-between items-center">
                 <div className="flex items-center gap-4"><div className="p-3 bg-white/10 rounded-2xl backdrop-blur-md"><ShieldCheck size={28} /></div><div><h3 className="text-xl font-black uppercase tracking-tighter">{isRtl ? 'إدارة عضوية البطاقة' : 'Card Membership Hub'}</h3><p className="text-[10px] font-bold text-blue-100 uppercase tracking-widest">{membershipEditingCard.name}</p></div></div>
              ...
              <div className="p-8 space-y-8 max-h-[70vh] overflow-y-auto no-scrollbar">
                 <div className="flex items-center justify-between p-6 bg-blue-50 dark:bg-blue-900/10 rounded-[2rem] border border-blue-100 dark:border-blue-900/30">
                    <div className="flex items-center gap-4"><Zap size={24} className={membershipForm.showMembership ? "text-blue-600" : "text-gray-400"} /><span className="text-xs font-black uppercase tracking-widest dark:text-white">{isRtl ? 'تفعيل قسم العضوية' : 'Enable Membership Section'}</span></div>
                    <button onClick={() => setMembershipForm({ ...membershipForm, showMembership: !membershipForm.showMembership })} className={`w-14 h-7 rounded-full relative transition-all ${membershipForm.showMembership ? 'bg-blue-600 shadow-lg' : 'bg-gray-300 dark:bg-gray-700'}`}><div className={`absolute top-1 w-5 h-5 bg-white rounded-full transition-all shadow-md ${isRtl ? (membershipForm.showMembership ? 'left-1' : 'left-8') : (membershipForm.showMembership ? 'right-1' : 'right-8')}`} /></button>
                 </div>
                 <div className={membershipForm.showMembership ? "space-y-6 opacity-100 transition-all" : "opacity-30 pointer-events-none transition-all"}>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                       <div className="space-y-2"><label className="text-[10px] font-black text-gray-400 uppercase tracking-widest px-1">{isRtl ? 'عنوان العضوية (عربي)' : 'Membership Title (AR)'}</label><input type="text" value={membershipForm.titleAr} onChange={e => setMembershipForm({ ...membershipForm, titleAr: e.target.value })} className="w-full px-6 py-4 rounded-2xl bg-gray-50 dark:bg-gray-800 border border-gray-100 dark:border-gray-700 font-bold text-sm dark:text-white outline-none focus:ring-4 focus:ring-blue-100" /></div>
                       <div className="space-y-2"><label className="text-[10px] font-black text-gray-400 uppercase tracking-widest px-1">{isRtl ? 'عنوان العضوية (EN)' : 'Membership Title (EN)'}</label><input type="text" value={membershipForm.titleEn} onChange={e => setMembershipForm({ ...membershipForm, titleEn: e.target.value })} className="w-full px-6 py-4 rounded-2xl bg-gray-50 dark:bg-gray-800 border border-gray-100 dark:border-gray-700 font-bold text-sm dark:text-white outline-none focus:ring-4 focus:ring-blue-100" /></div>
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                       <div className="space-y-2"><label className="text-[10px] font-black text-gray-400 uppercase tracking-widest px-1"><Clock size={10} className="inline mr-1" /> {isRtl ? 'تاريخ البدء' : 'Start Date'}</label><input type="date" value={membershipForm.startDate} onChange={e => setMembershipForm({ ...membershipForm, startDate: e.target.value })} className="w-full px-6 py-4 rounded-2xl bg-gray-50 dark:bg-gray-800 border border-gray-100 dark:border-gray-700 font-bold text-sm dark:text-white outline-none focus:ring-4 focus:ring-blue-100" /></div>
                       <div className="space-y-2"><label className="text-[10px] font-black text-gray-400 uppercase tracking-widest px-1"><Calendar size={10} className="inline mr-1" /> {isRtl ? 'تاريخ الانتهاء' : 'Expiry Date'}</label><input type="date" value={membershipForm.expiryDate} onChange={e => setMembershipForm({ ...membershipForm, expiryDate: e.target.value })} className="w-full px-6 py-4 rounded-2xl bg-gray-50 dark:bg-gray-800 border border-gray-100 dark:border-gray-700 font-bold text-sm dark:text-white outline-none focus:ring-4 focus:ring-blue-100" /></div>
                    </div>
                    <div className="pt-4 border-t dark:border-gray-800">
                       ...
                       <div className="grid grid-cols-3 gap-3">
                          <button onClick={() => handleQuickExtend(30)} className="py-4 bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-2xl font-black text-[10px] text-blue-600 hover:bg-blue-50 transition-all flex flex-col items-center gap-1"><CalendarPlus size={18} /><span>{isRtl ? '30 يوم' : '30 Days'}</span></button>
                          <button onClick={() => handleQuickExtend(90)} className="py-4 bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-2xl font-black text-[10px] text-indigo-600 hover:bg-indigo-50 transition-all flex flex-col items-center gap-1"><TrendingUp size={18} /><span>{isRtl ? '90 يوم' : '90 Days'}</span></button>
                          <button onClick={() => handleQuickExtend(365)} className="py-4 bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-2xl font-black text-[10px] text-amber-600 hover:bg-amber-50 transition-all flex flex-col items-center gap-1"><Star size={18} fill="currentColor" /><span>{isRtl ? 'سنة كاملة' : '1 Year'}</span></button>
                       </div>
                    </div>
                 </div>
              </div>
              <div className="p-8 pt-0 flex gap-3">
                 <button onClick={saveMembershipUpdates} disabled={isUpdatingMembership} className="flex-1 py-5 bg-blue-600 text-white rounded-3xl font-black text-xs uppercase shadow-0 flex items-center justify-center gap-3 hover:scale-[1.02] active:scale-95 transition-all disabled:opacity-50">{isUpdatingMembership ? <Loader2 size={18} className="animate-spin" /> : <Save size={18} />}{isRtl ? 'حفظ التغييرات' : 'Save Changes'}</button>
                 <button onClick={() => setMembershipEditingCard(null)} className="px-10 py-5 bg-gray-100 dark:bg-gray-800 text-gray-500 rounded-3xl font-black text-xs uppercase hover:bg-gray-200 transition-all">{isRtl ? 'إلغاء' : 'Cancel'}</button>
              </div>
           </div>
        </div>
      )}
      
      {sharingCard && <ShareModal data={sharingCard} lang={lang} onClose={() => setSharingCard(null)} />}
      ...
      {cardToDelete && (
        <div className="fixed inset-0 z-[1100] flex items-center justify-center p-4 bg-black/70 backdrop-blur-md animate-fade-in">
           <div className="bg-white dark:bg-gray-900 w-full max-sm md:max-w-md rounded-[3rem] shadow-0 border border-gray-100 dark:border-gray-800 overflow-hidden p-8 md:p-10 text-center space-y-6 animate-zoom-in">
              <div className="w-20 h-20 bg-red-50 dark:bg-red-900/20 text-red-600 rounded-full flex items-center justify-center mx-auto mb-4"><AlertTriangle size={40} /></div>
              <div className="space-y-2"><h3 className="text-xl font-black dark:text-white leading-relaxed">{isRtl ? "تأكيد حذف البطاقة" : "Confirm Card Deletion"}</h3><p className="text-xs font-bold text-gray-400 leading-relaxed px-4">{isRtl ? `هل أنت متأكد من حذف البطاقة "${cardToDelete.name}"؟ لا يمكن التراجع عن هذا الإجراء.` : `Are you sure you want to delete "${cardToDelete.name}"? This action cannot be undone.`}</p></div>
              <div className="flex flex-col gap-3 pt-4 items-center"><button onClick={handleDeleteConfirm} className="w-full max-w-[280px] py-4 bg-red-600 text-white rounded-2xl font-black text-xs uppercase shadow-0 hover:brightness-110 transition-all flex items-center justify-center gap-2"><Trash2 size={18} /> {isRtl ? "نعم، حذف البطاقة" : "Yes, Delete Card"}</button><button onClick={() => setCardToDelete(null)} className="w-full max-w-[280px] py-4 bg-gray-50 dark:bg-gray-800 text-gray-400 rounded-2xl font-black text-[10px] uppercase hover:bg-gray-100 transition-all flex items-center justify-center gap-2"><X size={16} /> {isRtl ? "تراجع" : "Cancel"}</button></div>
           </div>
        </div>
      )}
    </div>
